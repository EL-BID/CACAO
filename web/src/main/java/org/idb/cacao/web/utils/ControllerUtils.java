...
/**
 * Utility methods for Controller implementations in the scope of this web
 * application
 * 
...
public class ControllerUtils {
	...
	/**
	 *  Indicates if actual running thread is from a junit test 
	 */
	...
	public static GenericResponse returnErrorsAsGenericResponse(BindingResult result) {
		...
		return new GenericResponse(String.join("\n", errors));
	}
	...
	public static String getDebugInfo(HttpServletRequest request) {
		...
		try {
			...
			for (String header_name : Collections.list(request.getHeaderNames())) {
				report.append("HEADER: ").append(header_name).append(" = ").append(request.getHeader(header_name))
						.append("\n");
			}
			Map<String, String[]> params = request.getParameterMap();
			if (params != null && !params.isEmpty()) {
				for (Map.Entry<String, String[]> entry : params.entrySet()) {
					report.append("PARAM: ").append(entry.getKey()).append(" = ")
	...
	public static String getDebugInfo(URI uri, HttpMethod method, HttpEntity<?> request) {
		...
		try {
			...
			for (Map.Entry<String, List<String>> entry : request.getHeaders().entrySet()) {
				report.append("HEADER: ").append(entry.getKey()).append(" = ")
						.append(String.join(",", entry.getValue())).append("\n");
			}
	...
	/**
	 * Execute a search procedure, returning whatever it finds. Capture error
	 * related to non existent index.
	 */
	...
	/**
	 * Any functional interace that returns a 'Page' of some type. It's used with
	 * {@link ControllerUtils#searchPage(SomeSearch) searchPage}
	 */
	@FunctionalInterface
	...
	/**
	 * Includes into model a flag indicating the user has already logged in. Useful
	 * for conditions in pages that are displayed differently in both logged and
	 * non-logged areas.
	 */
	...
	/**
	 * Returns the provided page size, if the first parameter is present, and
	 * temporarily stores this information in User object.<BR>
	 * If the information was not provided, verifies if we have stored this
	 * information in User object (temporarily) and returns it.<BR>
	...
	public static void setRunningTest(boolean test) {
		ControllerUtils.isRunningTest = test;
	}
	...
	/**
	 * Try to reduce the potential risk related to using an user entry for 'auto
	 * complete' purpose.<BR>
	 * Reduce the size if the entry is very big.<BR>
	 * Removes some special characters (such as parenthesis, brackets, dashes,
	 * quotes, etc.).
	 */
	...
	public static String redirectToPresentationWarning(Model model, MessageSource messageSource) {
		model.addAttribute("message",
				messageSource.getMessage("presentation.disabled.feature", null, LocaleContextHolder.getLocale()));
